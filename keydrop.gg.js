...
async function run() {
	...
	fs.readdirSync('./cookies').filter(filename => filename.endsWith('.json')).forEach(async filename => {
		...
		await page.goto('https://key-drop.gg/en/');
...
const redeemCodeOnPage = async (page, code) => {
	...
	*/
	...
	console.log(`[Macro] Redirecting to https://key-drop.gg/?code=${code}`);
	await Promise.allSettled([
	    page.goto(`https://key-drop.gg/?code=${code}`),
	    page.waitForNavigation({ waitUntil: 'domcontentloaded' })
    ]);

	await wait(1000);
	if(page.url() == `https://key-drop.gg/en/?code=${code}#payment/promocode`) {
