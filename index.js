...
console.log('\x1b[32m', 'Keydrop Autocode V5', '\x1b[0m');
console.log('\x1b[31m', 'If you see this but the app keeps crashing, refresh your cookies and check if your token is valid.', '\x1b[0m');
console.log('\x1b[31m', 'If the console dosent say account loaded it means your missing cookie file(s).', '\x1b[0m');
console.log('\x1b[37m', 'Check out our github wiki for the FAQ', '\x1b[0m');
...
const redeemCodeOnPage = async (page, code) => {
	...
	*/
	...
	.then(async () => {
		...
		await page.waitForSelector(config.selectors.golden_code_tab_button);
		await erroredWait(1000, 100);
		await page.click(config.selectors.golden_code_tab_button);

		await page.waitForSelector(config.selectors.code_input);
		await erroredWait(1000, 100);
		await page.click(config.selectors.code_input);

		await erroredWait(200, 50);
		await page.type(config.selectors.code_input, code, { delay: 25 });

		await erroredWait(200, 100);
		await page.click(config.selectors.apply_button);
